...
//                    alert(window.location.href)
                        ...
                        alert('В буфер обмена будет записан адрес ссылки для получения подарка, перешлите его адресатам.')
                        location.reload();
                    }
                },
            });
    ...
    // Помощь. Клик по вопросительному знаку
    $("#help").on('click', function(e){
        alert('- Чтобы получить предсказание выберите один из подарков под ёлкой.\n' +
                '- Чтобы поздравить, кликните снеговика, напишите сообщение, ' +
                'полученную ссылку передайте получателям.\n' +
                '- Для просмотра поздравления, перейдите по полученной ссылке, ' +
                 'откройте подарок под ёлкой.')
    });

    // Выбран подарок с предсказанием
    $("[name = 'gift']").on('click', function(e){
        $('#message').hide();  // Скрыть текстовое поле для набора сообщения
    ...
    $("#snowman").on('click', function(e){
        ...
        $('#message').show();  // Показать текстовое поле для набора сообщения
        // Установка ширины текстового поля
        var w = $('#img_frame').width();
        $('#message').width(w/1.8);
        $('#message').focus();
    });
    ...
    $(".ok").on('click', function(e){
        if ($('#message').is(':hidden')) {
            ...
            $('#help').show();  // Показать знак вопроса
            $('#frame_div').hide();  // Скрыть текст
    ...
    // Закрыть рамку с формой или текстом
